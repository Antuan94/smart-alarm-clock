...
## 🧠 10. Análisis de Arquitectura y Respuestas a Reglas de Negocio

Siguiendo las directrices de análisis del enunciado, se detallan las decisiones de ingeniería de software adoptadas para garantizar la coherencia interna del sistema:

* **¿Cómo se representa una repetición semanal?** En el diseño actual, las instancias de `Alarm` se evalúan mediante objetos de tiempo absoluto (`LocalTime`). Para una extensión de repetición semanal completa, se planifica el uso de un atributo de tipo `Set<DayOfWeek>` (utilizando el enumerado nativo de Java). El método `checkAlarms` validaría no solo la coincidencia horaria, sino si el día actual simulado está contenido en dicho conjunto.
* **¿Cómo evitar alarmas duplicadas y qué ocurre si dos suenan simultáneamente?** Para evitar duplicidades, el identificador único (`String id`) actúa como clave primaria. En lógicas de persistencia avanzadas o colecciones indexadas, se utilizaría un `Map<String, Alarm>` o un filtrado previo que impida registrar dos alarmas a la misma hora exacta. En caso de coincidencia simultánea (dos alarmas a las 07:30), el diseño estructurado en bucle de `AlarmManager.checkAlarms` las procesa de forma secuencial y limpia, agregando ambas a la lista de disparo simultáneo devuelta al ejecutable principal.
* **¿Cómo modelar sonidos y desacoplar responsabilidades?** Los sonidos se desacoplan por completo delegando sus propiedades a la clase especializada `SoundProfile`. La alarma no conoce cómo se gestiona un decibelio o un archivo de audio; solo mantiene la referencia asociativa al objeto que lo encapsula, cumpliendo el principio SOLID SRP.
* **¿Cómo implementar y modelar el Snooze (Posponer)?** El protocolo de postergación (*Snooze*) se implementa en la lógica del sistema interceptando el disparo de la alarma activa. Al solicitar una postergación (por ejemplo, de 5 minutos), el controlador calcula dinámicamente un desfase temporal mediante `alarma.getTime().plusMinutes(5)`, retrasando su siguiente evaluación en el planificador sin alterar permanentemente la hora base configurada originalmente por el usuario.
* **¿Qué atributos deben ser privados y qué métodos públicos?** Todos los estados primitivos y colecciones estructurales (`alarms`, `time`, `volume`, `vacationModeActive`) se han blindado como **estrictamente privados (`private`)** para mantener el principio de ocultación de información. Los métodos encargados de validar restricciones de entrada, procesar cálculos o extraer reportes de lectura limpia se declaran como **públicos (`public`)**, exponiendo una API controlada hacia el exterior del paquete.